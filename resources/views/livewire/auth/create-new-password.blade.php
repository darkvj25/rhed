<div class="relative flex justify-center w-full h-dvh">
    <div class="flex flex-col justify-center w-full mt-3 md:w-1/2">
        <div class="flex flex-col items-center justify-center max-w-sm gap-2 mx-auto">
            <img src="/logo/veritas_logo.png" class="w-20 h-20 ">
            <h1 class="text-xl font-bold">Veritas College of Irosin</h1>
        </div>
        <p class="text-center text-white">Please change your password to secure your account.</p>
        <form wire:submit.prevent="ResetCode">
         <div class="w-full p-2 md:w-full" >{{ $this->form }}

             <div class="px-2 mt-2">
                 @error('password') <span class="text-red-500 error" >{{ $message }}</span> @enderror
             </div>
             <button type="submit"  wire:click="$commit"  class="w-full py-2 mt-3 text-white rounded-lg hover:bg-green-300 bg-gradient-to-r from-green-700 to-green-500">
                 Create new password
             </button>
             <div class="flex flex-col justify-center w-full mt-2 text-center">
                 <a href="{{route('login')}}" class="underline hover:text-green-500">Login</a>
             </div>
         <div>
        </form>
